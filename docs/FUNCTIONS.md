...
本文对应 `vps-toolkit 2.1.2`。所有菜单操作都需要 root 权限；推荐使用 `sudo nb` 进入。确认提示统一为 `[y/N]`：输入 `y` 或 `Y` 执行，输入 `n` 或直接回车取消。所有二级菜单执行功能后会留在当前菜单，只有选择 `0` 才返回主菜单。
...
从主菜单选择更新或执行 `sudo nb --update` 都会进入上述流程。升级完成后不需要退出并重新输入 `nb`；屏幕上重新出现的主菜单已经来自新版本。
