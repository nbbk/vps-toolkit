...
git_safe() { git -c "safe.directory=$ROOT" -C "$ROOT" "$@"; }
RELEASE_REF="${VMT_RELEASE_REF:-HEAD}"
if [ "$RELEASE_REF" = HEAD ]; then
  VERSION="$(sed -nE 's/^VERSION="([0-9.]+)"$/\1/p' "$ROOT/vps-tool.sh" | head -n1)"
else
  git_safe rev-parse --verify "${RELEASE_REF}^{commit}" >/dev/null
  VERSION="$(git_safe show "${RELEASE_REF}:vps-tool.sh" | sed -nE 's/^VERSION="([0-9.]+)"$/\1/p' | head -n1)"
fi
...
git_safe archive --format=tar.gz --prefix=vps-toolkit/ -o "$OUT/$NAME" "$RELEASE_REF"
