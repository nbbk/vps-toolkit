...
- BBR 组合管理：独立显示并切换拥塞控制与默认队列算法，内置 `BBR + fq`、`BBR + fq_codel`、`CUBIC + fq_codel`、`CUBIC + fq`，并支持从内核实际可用列表选择拥塞算法。
...
- `BBR + fq` 是代理/节点的推荐组合；切换前检查内核可用算法，切换后同时验证拥塞控制和默认队列。原生 BBR 不替换内核；XanMod BBRv3 仅在官方支持的 x86_64 Debian/Ubuntu 版本开放，安装、更新和卸载均要求 `y/N` 确认，不会自动重启。
