...
VMT_INSTALL_DIR="$DEST" VMT_STATE_DIR="$STATE" VMT_BIN_DIR="$BIN" VMT_ASSUME_YES=1 VMT_NO_LAUNCH=1 \
  bash "$DEST/update.sh" --rollback
rolled_back="$(sed -nE 's/^VERSION="([0-9.]+)"$/\1/p' "$DEST/vps-tool.sh" | head -n1)"
[ "$rolled_back" = 0.0.1 ] || { echo "rollback mismatch: $rolled_back" >&2; exit 1; }
